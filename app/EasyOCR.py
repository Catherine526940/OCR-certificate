...
from easyocr.EasyocrRecognizer import OCR
...
class EasyOCR:
    ...
    def getResult(self, enhanced_image, raw_shape, angle):
        ...
        for (i, (name, x1, y1, x2, y2)) in self.template.iterrows():
            ...
            #替换
            result = re.sub(r'(\d)皿', r'\1㎡', result)
            result = re.sub(r'(\d)m', r'\1㎡', result)
        ...
        data2_json["丘权号"] = chanquan[-8: -1] if hao_index != -1 else chanquan[-7:]
